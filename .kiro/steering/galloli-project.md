...
17. **GeoChain (js/geo-chain.js)**: Módulo de pesaje automático por GPS. En APK Capacitor lee GPS desde `BleForegroundService` via `BleForeground.getLocation()`. En PWA usa `navigator.geolocation`. Detecta cliente más cercano en radio de 50m y registra venta automáticamente cuando peso > 3.50 lb es estable 1.5s. El `BleForegroundService` ahora tiene tipo `connectedDevice|location` para GPS en background.
18. **BleForegroundPlugin métodos**: `start()`, `stop()`, `updateWeight({weight})`, `getLocation()` → `{lat, lng, hasLocation}`, `getWeight()` → `{weight}`.
